...
| # | Blocker | Who | Effort |
|---|---|---|---|
| 1 | ~~Verify the `✱` claims~~ **Done 21 Sep — 6 verified, 1 failed.** The one that mattered is `boardIds` with **number** IDs (ADR-028), and the wasted per-board request is gone. Only `board_automations` is still unread, behind a default-off flag no paid tier depends on. Next probe: `npm run verify:live -- --probe-preview` (ADR-029). | You | **1 minute** |
...
**The account exists as of 21 Sep, which unblocks everything below that is
not code.** `docs/09-first-hour.md` is the runbook: token, find the right
