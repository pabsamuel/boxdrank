...
// Notion writes go through the throttled client (max ~3 req/s per integration),
...
export async function startBackfill(shopDomain: string): Promise<string | null> {
  ...
  if (!ctx) {
    return "Connect Notion and create the databases before running a sync.";
  }
...
async function runBackfill(shopDomain: string): Promise<void> {
  // Fresh context each run (token/page could have changed since startup).
  const ctx = await getSyncContext(shopDomain);
  if (!ctx) throw new Error("Notion connection is not configured");
