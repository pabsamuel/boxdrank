// Encrypts / decrypts the merchant's Notion integration token so it is
...
function getKey(): Buffer {
  ...
  if (!secret) {
    throw new Error(
      "ENCRYPTION_KEY (or SHOPIFY_API_SECRET) must be set to encrypt Notion tokens",
    );
  }
  // scrypt stretches whatever string we get into a proper 32-byte AES key.
  return crypto.scryptSync(secret, "notionsync-token-v1", 32);
}
...
export function decryptToken(stored: string): string {
  ...
  if (!ivHex || !tagHex || !dataHex) {
    throw new Error("Stored Notion token has an unexpected format");
  }
