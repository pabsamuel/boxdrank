...
NotionSync — Notion Order & Product Sync ("NotionSync", "the app") is
operated by **[FILL IN: your name or company]** ("we", "us"). This policy
explains what data the app touches when a merchant ("you") installs it on
a Shopify store, and what happens to that data.

## What the app does with your store's data

NotionSync copies your store's **orders** and **products** from Shopify
into **Notion databases in a workspace that you own and control**. The
synced fields are:
...
This data flows **through** our server to Notion; order and customer
...
- Your Notion internal integration secret, **encrypted at rest with
  AES-256-GCM**. It is never stored or logged in plain text.
- Which Notion page and databases you sync into, and your sync settings.
- A mapping table linking Shopify order/product IDs to Notion page IDs
  (IDs only — no names, emails, or order contents).
...
- **Notion** receives the synced rows — into your own workspace, under
  your own Notion account and Notion's terms.
- Our server and database are hosted on **Railway**.

## Data retention & deletion

- Uninstalling the app deletes your sessions, settings, encrypted Notion
...
- Rows already synced to your Notion workspace are yours and remain in
  your Notion account — delete them there if you wish.
- We honor Shopify's GDPR webhooks (`customers/data_request`,
  ...
  acknowledgment; your Notion workspace is under your control.
