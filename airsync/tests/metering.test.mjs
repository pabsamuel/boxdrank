...
// (SQLite) with Airtable mocked: 50 orders sync, the 51st pauses sync,
...
// holds a stale Shop row. (A bug here once let the cap slip; this is the
// regression test.)
//
...
// Mock Airtable: every write succeeds instantly.
let recordSeq = 0;
globalThis.fetch = async () =>
  new Response(JSON.stringify({ id: `recTEST${++recordSeq}` }), {
    status: 200,
...
try {
  ...
  await db.shop.create({
    data: {
      shop: SHOP,
      airtableToken: crypto.encryptToken("patTESTTOKEN"),
      airtableBaseId: "appTESTBASE",
      airtableBaseName: "Test",
      ordersTableId: "tblORDERS",
      productsTableId: "tblPRODUCTS",
      plan: "free",
    },
  });
  ...
  const outcomes = [];
  for (let i = 1; i <= 52; i++) {
    outcomes.push(await sync.syncOrder(ctx, String(i), { "Order number": `#${i}` }));
  }
  ...
  const upd = await sync.syncOrder(ctx, "1", { "Order number": "#1-edited" });
  ...
  const afterUpgrade = await sync.syncOrder(ctx, "53", { "Order number": "#53" });
  check("pro resumes new orders", afterUpgrade === "created", afterUpgrade);
  const updAfter = await sync.syncOrder(ctx, "1", { "Order number": "#1-edited" });
  check("pro resumes updates", updAfter === "updated", updAfter);

  // A second sync of the same order must UPDATE the same Airtable record,
